...
namespace NetworkIO.src
{
    static class Physics
    {
        ...
        public static Vector2 CalculateRepulsionForce(Vector2 position, Vector2 positionOther, float repulsionForce1, float repulsionForce2, float scale = 1)
        {
            return Vector2.Normalize(position - positionOther) * CalculateRepulsion(repulsionForce1, repulsionForce2, scale);
        }
        ...
        public static float CalculateRepulsion(float repulsionForce1, float repulsionForce2, float distance, float scale = 1)
        {
            if (distance < 10)
                distance = 10;
            return repulsionForce1 * repulsionForce2 / (float)Math.Pow(distance/scale, 1/100);
        }
