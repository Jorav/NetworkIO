...
namespace NetworkIO.src
{
    public abstract class Unit
    {
        ...
        protected float Friction { get; set; } // percent, where 0.1f = 10% friction
        public Vector2 TotalExteriorForce;
        protected float elasticity;
        public bool IsVisible { get; set; }
        public bool IsCollidable { get; set; }

        public Unit(Vector2 position, float rotation = 0, float mass = 1, float thrust = 1, float friction = 0.1f)
        {
        ...
        public virtual void Collide(Unit unit)
        {
            if (CollidesWith(unit))
            {
                float r = Vector2.Distance(Position, unit.Position);
                Vector2 directionalVector = Position - unit.Position;
                directionalVector.Normalize();
                TotalExteriorForce += Physics.CalculateCollisionRepulsion(Position, Velocity, Mass, unit.Position, unit.Velocity, unit.Mass, Math.Min(Math.Max(unit.Width, unit.Height), Math.Max(Width, Height)), elasticity, unit.elasticity); //OBS: might break after changes
            }
        }

        public abstract bool CollidesWith(Unit unit);

        public abstract void Draw(SpriteBatch sb, Matrix parentMatrix);

        public virtual object Clone()
        {
            Unit mNew = (Unit)this.MemberwiseClone();
